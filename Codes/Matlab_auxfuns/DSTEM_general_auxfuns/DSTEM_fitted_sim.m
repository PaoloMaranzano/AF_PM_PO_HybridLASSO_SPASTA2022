...
classdef DSTEM_fit_sim < handle
    ...
    methods
        function obj = DSTEM_fit_sim(stem_model)
        ...
        function fit_simulate(obj,nan_rate,nan_pattern_par)
            ...
            %% Debugging PM
            % obj.stem_model = o_model;
            % obj.stem_model = obj_stem_model;
            ...
            %%% Creating spatio-temporal parameters (sigma)
            ...
            if obj.stem_model.stem_par.p>0
                if obj.stem_model.stem_data.stem_datestamp.irregular==0
                    Z=DSTEM_fit_sim.ar1_sim(G,s_eta,T,mu0,sigma0);
                else
                    times=obj.stem_model.stem_data.stem_datestamp.stamp;
                    Z=DSTEM_fit_sim.ar1_irregular_sim(G,lambda,s_eta,times,mu0,sigma0);
                end
            end
            ...
            % PM: 13/02/2023
            % writematrix(W_eps,'ErrMat.csv');
            % W_eps = obj.error_matrix;
            ...
            for t=1:T
                ...
                % PM: simulating Y as a combination of X_beta
...
%         end
    ...
    methods (Static)
        function Z = ar1_sim(G,sigma_eta,T,mu0,sigma_0)
            ...
            % Z0 = zeros(length(DSTEM_obj_sim.stem_model.stem_par.G),1);
            Z(:,1)=Z0;
            for t=2:T+1
